...
class SpineAnnotationTool:
    ...
    def load_images(self):
        ...
        for img_path in image_files:
            ...
            img = Image.fromarray(img_array.astype(np.uint16), mode='I;16')
    ...
    def update_display(self):
        ...
        # Compute display normalization from the *whole stack* once
        if not hasattr(self, 'global_minmax'):
            all_vals = np.concatenate([np.ravel(np.array(im.convert('I'))) for im in self.images])
            self.global_minmax = (np.percentile(all_vals, 0.1), np.percentile(all_vals, 99.9))

        img_array = np.array(img, dtype=np.float32)
        vmin, vmax = self.global_minmax
        img_array = np.clip((img_array - vmin) / (vmax - vmin), 0, 1)
        img_disp = (img_array * 255).astype(np.uint8)
